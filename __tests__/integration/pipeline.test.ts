...
`;
  ...
  describe('Happy Path - Complete Pipeline', () => {
    ...
    it('should create output document file', async () => {
      ...
      // In test mode, files are redirected to test-results
      const testResultsPath = 'docs/testing/test-results/general-tests';
      ...
      // Verify file content
      const outputFile = path.join(testResultsPath, outputFiles[outputFiles.length - 1]);
    ...
    it('should include metadata in generated document', async () => {
      ...
      // Check document metadata - files are in test-results in test mode
      const testResultsPath = 'docs/testing/test-results/general-tests';
      const files = await fs.promises.readdir(testResultsPath);
      const outputFiles = files.filter(f => f.startsWith('patent-assignment-agreement-') && f.endsWith('.md'));
      expect(outputFiles.length).toBeGreaterThanOrEqual(1);
